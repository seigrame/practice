	...
	public class Arrow : MonoBehaviour
	{
		...
		void OnCollisionEnter( Collision collision )
		{
			if ( inFlight )
			{
                ...
                // Jelly 타겟 체크 추가
                bool hitJelly = collision.collider.gameObject.GetComponent<JellyMeshReferencePoint>() != null;
				...
				else
				{
                    ...
                    if (rbSpeed > 0.1f || hitJelly)
                    {
                        JellyMeshReferencePoint jellyCollider = collision.collider.gameObject.GetComponent<JellyMeshReferencePoint>();
                        if (jellyCollider != null)
                        {
                            //collision.collider.gameObject.transform.SendMessageUpwards("ApplyDamage", SendMessageOptions.DontRequireReceiver);
                            jellyCollider.ParentJellyMesh.GetComponent<SocialVR.JellyTarget>().SendMessageUpwards("ApplyDamage", SendMessageOptions.DontRequireReceiver);
                        }
                    }
				}
